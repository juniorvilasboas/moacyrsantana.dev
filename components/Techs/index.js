  ...
  return (
    <div className='relative border-2 rounded-xl shadow-lg px-8 md:px-14 pb-4 py-10 mt-6'>
      ...
      <p>
          ...
          {techs
            ...
            .map((tech) => {
              return (
                ...
                >
                  <img
                    className='h-12 inline-block mr-6'
                    src={`https://cdn.jsdelivr.net/gh/devicons/devicon/icons/${tech.name}/${tech.name}-${tech.ext}.svg`}
                  />
                </a>
              )
            })}
        ...
        <div className='grid grid-cols-5 my-8'>
          {techs
            .filter((tech) => tech.tipo === 'back')
            .map((tech, i) => (
              <TechItem key={tech.id} item={tech} />
            ))}
        </div>
        <span className='text-xl font-bold py-1 px-6 -ml-6 border-2 rounded-xl'>
          Banco de dados
