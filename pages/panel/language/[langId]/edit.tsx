...
const EditLanguage = () => {
  ...
  const { data } = useGet(
    router?.query?.langId && `/api/language/${router?.query?.langId}`
  )
  ...
  useEffect(() => {
    if (data) {
      ...
      setValue('levelUnderstand', data?.levelUnderstand.id)
      setValue('levelSpeak', data?.levelSpeak.id)
      setValue('levelWrite', data?.levelWrite.id)
    }
  }, [data, setValue])

  const onSubmit: SubmitHandler<NewLanguageForm> = async (inputs) => {
    const data = await patch({
      url: `/api/language/${router?.query?.langId}`,
      data: inputs
    })
